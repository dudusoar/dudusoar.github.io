...
author: "Yuchen Du"
description: "A two-stage LLM agent that uses RAG with graph-based memory to ground fraud-related seller-appeal recommendations in evidence, verification actions, and historical human corrections."
summary: "We transform historical reviewer corrections into retrievable Evidence-Action-Factor-Decision graphs, then use them to ground recommendations for new seller appeals. The deployed system achieved 96.3% cumulative alignment with human experts in production."
cover:
  image: "cover.png"
...
Large e-commerce platforms use fraud-detection controls to restrict seller accounts that appear risky. Some sellers appeal those restrictions, requiring investigators to reconstruct fragmented evidence, verification actions, and prior review decisions before deciding whether an account should be reinstated or remain blocked.

In the historical review process, a first reviewer — the **Maker** — examines the available evidence and makes a decision. A second reviewer — the **Checker** — audits that decision. Sometimes, the Checker overturns it.
...
We built a **two-stage LLM agent** that uses retrieval-augmented generation (RAG) with graph-based memory: offline, it converts historical evidence, verification actions, review factors, and decisions into structured graphs; online, it retrieves similar correction patterns to analyze and recommend outcomes for new appeals.
...
The knowledge base aggregates conflict-annotated graphs from historical reviewed cases, forming a retrievable repository of adjudication precedents.

## Online Reasoning

In the online stage, the agent follows a four-phase reasoning process:
...
- industrial deployment of AI decision-support systems.
