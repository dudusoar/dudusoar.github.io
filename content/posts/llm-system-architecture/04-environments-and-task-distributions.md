...
description: "Reliable evaluation separates plausible executable environments from meaningful LLM task distributions."
...
Separate scenario design for an LLM decision system into two layers.
...
Together, the two layers form this structure:
...
An environment contract makes at least the following properties reproducible:
...
These rules belong to the environment rather than to a particular LLM method. A task family must not temporarily override domain constraints simply to create a favorable example.

## The Task-Distribution Contract

The task-distribution layer defines:
...
A task distribution is more than a list of case IDs. It is a research and evaluation distribution:
...
Do not extract offline experience indiscriminately from a pool of semantically undefined random cases. Tie each experience to a task family with stable applicability conditions.

Useful memory descriptors include:
...
Sample held-out evaluations from defined task families. A useful suite often includes:
...
Without the first, an evaluation may reward behavior that works only in a toy setup. Without the second, an average performance number may have no mechanism-level meaning. Reliable LLM evaluation needs both.
